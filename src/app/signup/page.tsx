...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Create user document in Firestore (will check if exists)
      await createUserDocument(user.uid, user.email, user.displayName);

      router.push('/dashboard');

    } catch (err) {
      // ... (Keep existing Google error handling from LoginPage) ...
  ...
  const handleFacebookSignup = async () => {
      // ... (Keep existing placeholder Facebook logic from LoginPage) ...
  };
  ...
  return (
    <div className="min-h-screen flex font-sans">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-gray-100 p-4 md:p-8">
        <div className="relative w-full max-w-sm">
          <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 z-20">
            <div className="bg-green-600 rounded-full p-3 border-4 border-white shadow-lg"> <LeafLogo /> </div>
          </div>
          <div className="relative bg-white rounded-xl shadow-lg px-8 pt-16 pb-8 mt-8">
            ...
            <form onSubmit={handleEmailPasswordSignup} className="space-y-4">
              ...
              <div>
                ...
                <input id="displayName" name="displayName" type="text" required value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="w-full px-3 py-2 border border-gray-300 text-gray-700 rounded-md focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 transition duration-200" disabled={isLoading || isGoogleLoading || isFacebookLoading} />
              </div>
              {/* Email */}
              <div>
              ...
              <div className="relative">
                ...
                <input id="password" name="password" type={showPassword ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2 border text-gray-700 border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 transition duration-200 pr-10" disabled={isLoading || isGoogleLoading || isFacebookLoading} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 top-6 flex items-center px-3 text-gray-400 hover:text-green-600" aria-label={showPassword ? 'Hide password' : 'Show password'} disabled={isLoading || isGoogleLoading || isFacebookLoading}> {showPassword ? <EyeOff size={18} /> : <Eye size={18} />} </button>
              </div>
              {/* Confirm Password */}
              <div className="relative">
                <label htmlFor="confirmPassword" className="block text-sm font-medium text-gray-700 mb-1">Confirm Password</label>
                <input id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? 'text' : 'password'} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-3 py-2 border text-gray-700 border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 transition duration-200 pr-10" disabled={isLoading || isGoogleLoading || isFacebookLoading} />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute inset-y-0 right-0 top-6 flex items-center px-3 text-gray-400 hover:text-green-600" aria-label={showConfirmPassword ? 'Hide password' : 'Show password'} disabled={isLoading || isGoogleLoading || isFacebookLoading}> {showConfirmPassword ? <EyeOff size={18} /> : <Eye size={18} />} </button>
              </div>

              {error && ( <div className="text-red-600 text-sm text-center p-2 bg-red-50 rounded-md border border-red-200"> {error} </div> )}
              ...
              {/* Login Link */}
              <p className="text-center text-sm text-gray-600"> Already have an account?{' '} <Link href="/login" className="font-medium text-green-700 hover:text-green-800 hover:underline"> Login </Link> </p>

              {/* Social Signups */}
              <div className="space-y-3 pt-4">
                <button type="button" onClick={handleGoogleSignup} disabled={isLoading || isGoogleLoading || isFacebookLoading} className={`w-full inline-flex justify-center items-center py-2.5 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-200 ${(isLoading || isGoogleLoading || isFacebookLoading) ? 'opacity-50 cursor-not-allowed' : ''}`}> {isGoogleLoading ? ( /* Spinner */ <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-green-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"> <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle> <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg> ) : <GoogleLogo /> } {isGoogleLoading ? 'Processing...' : 'Sign up with Google'} </button>
                <button type="button" onClick={handleFacebookSignup} disabled={isLoading || isGoogleLoading || isFacebookLoading} className={`w-full inline-flex justify-center items-center py-2.5 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-200 ${(isLoading || isGoogleLoading || isFacebookLoading) ? 'opacity-50 cursor-not-allowed' : ''}`}> {isFacebookLoading ? ( /* Spinner */ <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"> <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle> <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg> ) : <FacebookLogo /> } {isFacebookLoading ? 'Processing...' : 'Sign up with Facebook'} </button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="hidden md:block md:w-1/2 bg-gray-200">
         <img src="https://placehold.co/800x1000/a3e6b7/4d7c0f?text=Greenhouse+Image\n(Replace+Me)" alt="Greenhouse background" className="w-full h-full object-cover" />
      </div>
    </div>
  );
}
